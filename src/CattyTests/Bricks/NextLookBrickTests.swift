...
final class NextLookBrickTests: AbstractBrickTest {
    ...
    func testNextLookBrickWithCache() {
        ...
        XCTAssertEqual(spriteNode.currentLook, lookB, "NextLookBrick not correct")
        XCTAssertEqual(0, imageCacheMock.loadImageFromDiskCalledPaths.count)
    }

    func testNextLookBrickWithoutCache() {
    ...
    func testNextLookBrickWithOneLook() {
        let imageCacheMock = RuntimeImageCacheMock(imagesOnDisk: [lookB.path(for: scene)!: image],
                                                   cachedImages: [])
    ...
    func testNextLookBrickWithNoLook() {
        let imageCacheMock = RuntimeImageCacheMock(imagesOnDisk: [lookB.path(for: scene)!: image],
                                                   cachedImages: [])
